...
\begin{enumerate}[resume = steps]
	...
	\item Check whether any spawning has already occured. If keeping mussels in a communal container, promptly isolate individuals in separate cups/beakers as soon as they start spawning, taking care of carefully wash them with FSW/FASW and air dry. If no spawning can be observed, repeat from step 4.
\end{enumerate}
...
\subsection*{\qty{3.2}{\percent} paraformaldehyde (PFA) fixative}

\begin{table}[H]
	%\setstretch{0.95}
	\centering
	\begin{tabular}{r
		S[table-format = 4.4, exponent-mode = fixed, fixed-exponent = 0, round-mode = places, round-precision = 4]@{\,} % disable scientific notation
		l
		S[table-format = 3.2, table-align-text-post = false, exponent-mode = fixed, fixed-exponent = 0, round-mode = places, round-precision = 2] % disable scientific notation
		c
		}
		...
		Paraformaldehyde						 & 3.2									 & \unit{\g}																							  & \NA																												& \qty{3.2}{\percent} 	   \\
		\qty{1}{\molar} \ce{NaOH}				 & \multicolumn{2}{c}{as needed}																											  	  & \NA																										    	& \NA				       \\
		\qty{1}{\molar} \ce{HCl}				 & \multicolumn{2}{c}{as needed}																											  	  & \NA																										    	& \NA				       \\
		\pbs			                         & \multicolumn{2}{c}{\NA}					                                                                                                      & \NA																												& \NA				       \\
	\end{tabular}
\end{table}

\begin{enumerate}
	\item Under a ventilated hood, add \qty{80}{\ml} of \pbs{} into a glass beaker and preheat at \qty{60}{\degreeCelsius} (mind that the solution does not boil);
	\item while stirring, careefully add \qty{3.2}{\g} of PFA;
	\item gently raise the pH by adding \qty{1}{\molar} \ce{NaOH} dropwise with a glass pipette: this will speed up PFA solubilization;
	\item when PFA is fully dissolved and the solution is clear, adjust the volume to \qty{100}{\ml};
	\item \textit{(Optional)} Filter the solution with filter paper disks;
	\item if necessary, adjust the pH to 7 by adding \qty{1}{\molar} \ce{HCl} dropwise with a glass pipette;
	\item the fixative solution can be stored for short-term usage at \fourdegree{} or for long-term usage at \minustwenty{}, possibly protected from light.
\end{enumerate}
